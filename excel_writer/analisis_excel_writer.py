...
# # GENERATE EXCEL
def generate_excel( client_name: str,
                    client_rut: str,
                    account: str,
                    user_date: datetime,
    ...
    usDollar =  indicators.get('dolar')
    uf = indicators.get('uf')
    cm = indicators.get('ipc')
    utmMonth =  indicators.get('utm')
    accountCode = account +' Cliente'
    emitDate = user_date.strftime('%d/%m/%Y')
    ...
    worksheet.insert_image(0, 7, file_path+'\\logo_addval.png', {'x_scale': 1, 'y_scale': 1})
    ...
    worksheet.write(row + 0, column + 1, accountCode, top_border)
    worksheet.write(row + 1, column + 0, 'U.F Cierre:', left_border)
    worksheet.write(row + 1, column + 1, uf)
    worksheet.write(row + 2, column + 0, 'TC Dólar Cierre:', bottom_left_border)
    worksheet.write(row + 2, column + 1, usDollar, bottom_border)
    ...
    worksheet.write(row + 1, column + 7, cm, right_border)
    worksheet.write(row + 2, column + 6, 'U.T.M:', bottom_border)
    worksheet.write(row + 2, column + 7, utmMonth, bottom_right_border)
    ...
    worksheet.write(row + 4, column + 7, int(movimientos_df.Saldo.sum()), bottom_border_currency)

    # Define variable to breakline
    breakColumn = 7 + row

    # for client in range(len(dataToArray)):
    for rut,cliente in movimientos_df.groupby('Rut', sort=False):
